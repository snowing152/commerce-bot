...
export type FilterInput =
  | string
  | { group?: string; value: string; minScore?: number };
...
export interface FiltersConfig {
  panel: string;
  expandButton: string;
  groups: Record<string, FilterGroupConfig>;
  state: {
...
export class AutomationEngine {
  ...
  private getFiltersConfig(): FiltersConfig | null {
    return this.selectors.filters || null;
  }
  ...
  private toQuery(
    ...
    if (typeof f === "string") return { value: f, minScore: 0.6 };
    return { group: f.group, value: f.value, minScore: f.minScore ?? 0.6 };
  }
  ...
  private scoreMatch(
    ...
    if (!inputNorm || !optNorm) return 0;
    if (inputNorm === optNorm) return 1.0;
    if (optNorm.includes(inputNorm) || inputNorm.includes(optNorm)) return 0.85;
    for (const [k, vs] of Object.entries(aliases)) {
      const kNorm = this.normalizeLabel(k);
      const vNorms = vs.map((v) => this.normalizeLabel(v));
      if (
        (inputNorm === kNorm && vNorms.includes(optNorm)) ||
        (optNorm === kNorm && vNorms.includes(inputNorm)) ||
        (vNorms.includes(inputNorm) && vNorms.includes(optNorm))
      ) {
        return 0.95;
      }
    }
    const ta = new Set(inputNorm.split(" ").filter(Boolean));
    const tb = new Set(optNorm.split(" ").filter(Boolean));
  ...
  private bestMatch(
    q: { group?: string; value: string },
    snap: FilterSnapshot,
    aliases: Record<string, string[]>,
  ): FilterMatch {
    const inputNorm = this.normalizeLabel(q.value);
    const candidates = q.group
      ? snap.options.filter((o) => o.group === q.group)
      : snap.options;
    let best: FilterOption | null = null;
    let bestScore = 0;
    let runnerUp = 0;
    for (const o of candidates) {
      const s = this.scoreMatch(inputNorm, o.normalized, aliases);
      if (s > bestScore) {
        runnerUp = bestScore;
        bestScore = s;
        best = o;
    ...
    return {
      option: best,
      score: bestScore,
      ambiguous: bestScore > 0 && bestScore - runnerUp < 0.05,
    };
  }

  private async takeFilterSnapshot(
    page: Page,
    cfg: FiltersConfig,
  ): Promise<FilterSnapshot> {
    const raw = await page.evaluate((c: FiltersConfig) => {
      const panel = document.querySelector(c.panel);
      if (!panel) return { contentHash: "", options: [] };
      const out: Array<{
        id: string;
        label: string;
        group: string;
      ...
      for (const [groupName, g] of Object.entries(c.groups)) {
        const root = panel.querySelector(g.root);
        if (!root) continue;
        root.querySelectorAll(g.option).forEach((el) => {
          ...
          const ariaSel = el.getAttribute(c.state.ariaSelected);
          const ariaDis = el.getAttribute(c.state.ariaDisabled);
          out.push({
            id,
            label: (el.innerText || "").trim(),
            group: groupName,
            selected:
              cls.includes(c.state.selectedClass) ||
              liCls.includes(c.state.selectedClass) ||
              ariaSel === "true",
            disabled:
              cls.includes(c.state.disabledClass) ||
              liCls.includes(c.state.disabledClass) ||
              ariaDis === "true",
          });
        });
      }
      const hash =
        String(out.length) +
        ":" +
        String(out.map((o) => o.label).join("|").length);
      return { contentHash: hash, options: out };
    }, cfg);

    const options: FilterOption[] = raw.options.map((o) => ({
      ...o,
      normalized: this.normalizeLabel(o.label),
    }));
    return { contentHash: raw.contentHash, options };
  }

  private async getFilterContentHash(
    page: Page,
    cfg: FiltersConfig,
  ): Promise<string> {
    return this.safeExecute(
      page.evaluate((c: FiltersConfig) => {
        const panel = document.querySelector(c.panel);
        if (!panel) return "";
        const opts: string[] = [];
        for (const g of Object.values(c.groups)) {
          ...
          if (!root) continue;
          root.querySelectorAll(g.option).forEach((el) => {
            if (el instanceof HTMLElement) opts.push(el.innerText.trim());
          });
        }
        return String(opts.length) + ":" + String(opts.join("|").length);
      }, cfg),
      "getFilterContentHash",
      "",
    );
  }

  private async expandAllFilters(panel: Locator, expandSelector: string) {
    const moreBtns = panel.locator(expandSelector);
    for (let pass = 0; pass < 6; pass++) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        try {
          ...
          this.logStep(
            "DEBUG",
            `Could not click "more filters" ${i}: ${errMessage}`,
            "expandAllFilters",
          );
        }
      }
  ...
  private async applyFilters(page: Page, filters: FilterInput[]) {
    if (!filters?.length) return;
    const cfg = this.getFiltersConfig();
    if (!cfg) {
      this.logStep(
        "WARN",
        "selectors.filters config missing. Skipping filter application.",
        "applyFilters",
      );
      return;
    }

    const queries = filters.map((f) => this.toQuery(f));
    this.logStep(
      "INFO",
      `Applying filters: ${queries.map((q) => q.value).join(", ")}`,
      "applyFilters",
    );

    const panel = page.locator(cfg.panel).first();
    ...
    if (panelCount === 0) {
      this.logStep("WARN", "Filter panel not found.", "applyFilters");
      return;
    }

    await Humanizer.wait(800, 1400);
    await this.expandAllFilters(panel, cfg.expandButton);
    let snap = await this.takeFilterSnapshot(page, cfg);
    this.logStep(
      "INFO",
      `SNAPSHOT options=${snap.options.length} hash=${snap.contentHash}`,
      "applyFilters",
    );

    const anyMissing = queries.some(
      (q) => this.bestMatch(q, snap, cfg.aliases).score < q.minScore,
    );
    if (anyMissing && snap.options.length > 0) {
      await this.expandAllFilters(panel, cfg.expandButton);
      snap = await this.takeFilterSnapshot(page, cfg);
      this.logStep(
        "INFO",
        `SNAPSHOT (after retry-expand) options=${snap.options.length} hash=${snap.contentHash}`,
        "applyFilters",
      );
    }
    ...
    for (const q of queries) {
      ...
      const m = this.bestMatch(q, snap, cfg.aliases);

      if (m.score < q.minScore || !m.option) {
        this.logStep(
          "WARN",
          `FILTER_MISS keyword="${q.value}" best="${m.option?.label ?? "(none)"}" score=${m.score.toFixed(2)} group=${q.group ?? "*"}`,
          "applyFilters",
        );
        continue;
      }
      if (m.option.disabled) {
        this.logStep(
          "WARN",
          `FILTER_DISABLED keyword="${q.value}" label="${m.option.label}"`,
          "applyFilters",
        );
        continue;
      }
      if (m.option.selected) {
        this.logStep(
          "DEBUG",
          `Filter already selected: "${m.option.label}"`,
          "applyFilters",
        );
        continue;
      }
      if (m.ambiguous) {
        this.logStep(
          "DEBUG",
          `FILTER_AMBIGUOUS keyword="${q.value}" picked="${m.option.label}" group=${m.option.group}`,
          "applyFilters",
        );
      }

      const loc = page.locator(`[data-bot-id="${m.option.id}"]`).first();
      await loc.scrollIntoViewIfNeeded().catch(() => undefined);
      await Humanizer.move(page, loc);

      try {
        await loc.click({ timeout: 2500 });
      } catch (e: any) {
        const msg = e instanceof Error ? e.message : String(e);
        if (/intercepts pointer events|not visible|outside of the viewport/i.test(msg)) {
          try {
            await loc.click({ timeout: 1500, force: true });
          } catch (forceErr: any) {
            this.logStep(
              "ERROR",
              `FILTER_CLICK_FAIL keyword="${q.value}" label="${m.option.label}" reason="${forceErr.message}"`,
              "applyFilters",
            );
            continue;
          }
          ...
          this.logStep(
            "ERROR",
            `FILTER_CLICK_FAIL keyword="${q.value}" label="${m.option.label}" reason="${msg}"`,
            "applyFilters",
          );
          continue;
        }
      ...
      this.logStep(
        "SUCCESS",
        `FILTER_HIT keyword="${q.value}" label="${m.option.label}" group=${m.option.group} score=${m.score.toFixed(2)}`,
        "applyFilters",
      );
      ...
      const fresh = await this.getFilterContentHash(page, cfg);
      if (fresh && fresh !== snap.contentHash) {
        snap = await this.takeFilterSnapshot(page, cfg);
      }
    }
  }
  ...
  private async applyCost(page: Page, costFilters: string[]) {
    ...
    return this.applyFilters(
      page,
      costFilters.map((v) => ({ group: "price", value: v })),
    );
  }
  ...
  async run(): Promise<string | null> {
    ...
    try {
      ...
      for (const task of this.config.tasks) {
        ...
        // BETA: filter system is disabled for test build (encoding-corrupted
        // user configs were producing FILTER_MISS for every input). Re-enable
        // by uncommenting once config.json encoding is fixed.
        if (task.filters?.length || task.cost?.length) {
          this.logStep(
            "SKIP",
            `Filters disabled in beta build. Skipping ${(task.filters?.length || 0) + (task.cost?.length || 0)} filter(s).`,
            "run",
          );
        }
