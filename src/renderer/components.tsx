...
const iconPaths: Record<string, React.ReactNode> = {
  ...
  won:          <><path d="M3 6h18l-2 13H5L3 6z"/><path d="M3 6L2 3"/></>,
...
/* ---------- StatusBadge ---------- */
type BadgeStatus = 'waiting' | 'confirmed' | 'success' | 'active' | 'running' | 'error' | 'expired' | 'idle' | 'warn' | 'info'
...
export const StatusBadge = ({ status, label, pulse = false, size = 'md' }: StatusBadgeProps) => {
  ...
  return (
    <span className={`inline-flex items-center font-medium tracking-tight rounded-full border ${sz} ${c.bg} ${c.text}`}>
      <span className={`relative w-1.5 h-1.5 rounded-full ${c.dot}`}>
        {pulse && <span className={`absolute inset-0 rounded-full ${c.dot} animate-ping opacity-60`} />}
      </span>
...
export const LogLine = React.memo(({ time, level, message, source }: LogLineProps) => (
  ...
  >
    ...
    <span className={`shrink-0 w-[58px] font-semibold tracking-[0.04em] ${levelColor[level] ?? 'text-zinc-400'}`}>
      {level}
    </span>
...
export const ProgressBar = ({ value = 0, indeterminate = false, label, className = '' }: ProgressBarProps) => (
  <div className={`flex items-center gap-2.5 ${className}`}>
    ...
    {!indeterminate && <span className="text-[11px] text-zinc-400 tabular-nums w-9 text-right">{Math.round(value)}%</span>}
  </div>
)
...
export const SectionHeader = ({ title, subtitle, right, className = '' }: SectionHeaderProps) => (
  <div className={`flex items-center justify-between px-4 h-10 border-b border-zinc-800/70 ${className}`}>
...
export const Wordmark = ({ size = 'md' }: WordmarkProps) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`${dim} relative rounded-md bg-gradient-to-b from-zinc-200 to-zinc-400 text-zinc-950 grid place-items-center font-bold tracking-tighter shadow-[0_1px_0_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.4)]`}>
      ...
      <div className="flex flex-col leading-none">
        ...
        {size === 'lg' && <span className="text-[10.5px] text-zinc-500 mt-0.5 tracking-tight">Automation Client</span>}
      </div>
    </div>
  )
}
