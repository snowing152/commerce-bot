...
describe('normalizeName product matching', () => {
  // Mirrors the private helper in AutomationEngine.run() exactly.
  ...
  it('matches a card whose name contains the full target name', () => {
    ...
    expect(normalizeName(cardName).includes(normalizeName(target))).toBe(true);
  });
  ...
  it('does NOT match a card that only shares a 4-word prefix with the target', () => {
    ...
    expect(normalizeName(otherCard).includes(normalizeName(target))).toBe(false);
  });
